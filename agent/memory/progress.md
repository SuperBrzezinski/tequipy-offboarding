...
- `fix(dev-038)` — test quality audit: fixed 4 TypeScript errors in spec files left stale by dev-035 refactor (`getSessionItems`/`initSession` removed from employee-list mock, `beginReturn`/`beginIssue` 2-arg calls → 1-arg in guard spec, `mockImplementation` return type for `ConfirmationService.confirm`, unsafe double-cast in repository spec); added `typecheck` Nx target to all 4 projects and `Typecheck` step to CI — TypeScript errors in spec files are now blocking.

- `docs(dev-037)` — documented Q8 assumption in discovery.md and ADR-0002: `Returned` and `Issue` are mutually exclusive states; `returnCondition` (Damaged/Missing) is the mechanism for flagging problems at physical return; `Issue` is reserved for absent/uncertain physical returns. Confirmed by operator 2026-06-01.
...
- `refactor(dev-031)` — Code review fixes (blocker + top 3): merged duplicate `@org/offboarding-feature/data-access` import statement in session-page component; introduced discriminated union for `ReturnItem` (`'Returned'` requires non-optional `returnCondition`, `'Pending'|'Issue'` keep it optional); updated `offboarding.store.ts` — `loadSession` uses conditional object construction (pre-completed employees get `returnCondition: ai.assignedCondition`), `confirmReturn`/`undoReturn`/`confirmIssue` use explicit `{ item, status, ... }` construction instead of `{ ...spread }` on union type to avoid TS unsound widening; removed `!` template assertion in equipment-row via `@if (item().returnCondition; as rc)`; replaced `store['_editingItem'].set(...)` bracket-notation in guard spec with `store.beginReturn/beginIssue` public API; removed `protected` from `selectedCondition`/`noteValue` signals in equipment-row component (template-bound state, now explicitly accessible); replaced `['selectedCondition']` bracket bypass with `.selectedCondition.set()`; refactored `confirmIssue` test to use `userEvent.type` on textarea; removed dead `makeIssueItem` fixture.
